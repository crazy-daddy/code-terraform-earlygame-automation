...
| **Phase 1** | **1.0 ppt $\text{O}_2$** | Auto Feeders (`research_auto_feeders`) | **Power ON Bio-Loop**: Collector, Lab, and Exchange now have container transfer support. Earns one-time Bio Order credits (plus Feed Maker/Fertilizer blueprint unlocks) from Day 1. No Terraform Index: biomass TP comes from the later Biosphere Mixer track. |
...
| **Phase 4** | **100,000 TP** | Pioneer Chassis (`research_pioneer`) | Tri-Pillar complete (~98.6k TP; generators keep running to close the last ~1.4k to **100,000 TP**. Bio-Loop adds credits only). Assemble Pioneer, mount Nav + basic Sonar + 6x Battery Holder; runs the standalone Scout role (survey-only, no `lib/`) until Phase 5 hands it the full `PioneerController`. |
